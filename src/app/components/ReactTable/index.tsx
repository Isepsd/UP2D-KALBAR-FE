...
const ReactTable: FC<Props> = ({
  ...
  const getColumnStyle = (column: any) => {
    if (
      column.pinned ||
      (rowSelectType === "radio" && column.accessor === "selection")
    ) {
      return {
        position: "sticky",
        left: 0,
  ...
  useEffect(() => {
    onCheckedRows &&
      onCheckedRows(selectedFlatRows.map((d: any) => d.original));
  }, [selectedFlatRows]);
    ...
    <>
      ...
      >
        ...
        >
          <StyledTable
            {...getTableProps()}
            style={{ minWidth: "100%", maxWidth: "max-content" }}
          >
            <thead>
                          ...
                          /> */}
                            {[
                              ...
                              <select
                                value={filterValues[column.id] || ""}
                                onChange={(e: any) =>
                                  handleColumnFilterChange(column, e)
                                }
                                style={{
                                  width: "100%",
                                  padding: "0.5rem 0.5rem 0.5rem 30px",
                                  border:
                                    "1px solid var(--input-border-color, #ddd)",
                                  borderRadius: "4px",
                                  backgroundColor: "var(--white)",
                                  color: "var(--black)",
                                }}
                ...
                return (
                  <tr {...row.getRowProps()} key={`tbody.tr${i}`}>
                    {row.cells.map((cell: any, indexTd: any) => {
                      ...
                      const release_rotbmh = row.original.release_rotbmh;

                      // Conditional style logic for background color
                      let backgroundColor = "";
                      if (approvel_area === 1 && approvel_apd === 0) {
                        backgroundColor = "#FFFE99"; // Soft yellow (but not too pastel) for approvel_area = 1 and approvel_apd = 0
                      } else if (approvel_area === 1 && approvel_apd === 1) {
                        backgroundColor = "#81C784"; // Soft green (but not too pastel) for approvel_area = 1 and approvel_apd = 1
                      } else if (approvel_area === 1 && approvel_apd === 2) {
                        backgroundColor = "#FA8072"; // Salmon red for status_pekerjaan = "BATAL DILAKSANAKAN"
                      } else if (release_rotbmh === 1) {
                        backgroundColor = "#c4a7fa"; // Salmon red for status_pekerjaan = "RELEASE PEKERJAAN"
                      }
                        ...
                        <td
                          ...
                          style={{
                            ...(cell?.column?.pinned && {
                              ...
                              zIndex: 10,
                              background: "var(--white)", // Tambahkan background yang konsisten
                            }),
                            backgroundColor: backgroundColor || "var(--white)", // Fallback jika backgroundColor kosong
                          }}
                        >
                          {cell.render("Cell")}
                        </td>
