...
import { PENYULANG_MODAL_COLUMN } from '@app/configs/react-table/master-jaringan.columns.config';
...
const ModalPenyulang: FC<Props> = ({
  ...
  const [columns] = useState<any>(PENYULANG_MODAL_COLUMN());
  ...
  useEffect(() => {
    setModal({ ...modalProps });
    if(modalProps?.show && gighSelected) getAllData()
  ...
  const getAllData = async () => {
    ...
    try {
      const params = {
        ...
        id_gardu_induk: gighSelected?.id_ref_jenis_lokasi?.id_ref_jenis_lokasi == 4 && gighSelected.id_ref_lokasi,
        id_gardu_hubung: gighSelected?.id_ref_jenis_lokasi?.id_ref_jenis_lokasi == 11 && gighSelected.id_ref_lokasi,
        ...reqFilters
      };
    ...
    <>
      ...
      <Modal keyboard={false} backdrop='static' scrollable centered show={modal.show} onHide={modalDecline} size='xl'>
        <Modal.Header closeButton>
          <Modal.Title className='w-100'>Daftar Penyulang</Modal.Title>
