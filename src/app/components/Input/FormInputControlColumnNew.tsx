...
const FormInputControlColumn: React.FC<{
  ...
  column2?: number;
  readOnly?: boolean;
  onChange?: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void; // Add onChange here
}> = ({
  labelName,
  required = false,
  placeholder,
  ...
  column2 = 8,
  readOnly = false,
  onChange, // Capture the onChange event
}) => {
  return (
    <Form.Group as={Row} className={className}>
      ...
      <Col md={column2}>
        <Form.Control
          ...
          readOnly={readOnly}
          onChange={onChange} // Pass onChange to Form.Control
        />
