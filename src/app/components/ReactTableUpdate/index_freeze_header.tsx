    ...
    <>
      ...
      >
        {/* <PerfectScrollbar className="mb-3" style={{ height: '100%' }}> */}
          {/* <div className="mb-3" style={{ height: '100%', overflowY: 'auto', overflowX: 'auto' }}></div> */}
                      ...
                      {columnFilters && !column.disableFilters && column.accessor && (
                        ...
                        >
                          ...
                          {column.id === 'butuh_padam' ? (
                            ...
                            <input
                              type={column.id.includes('datum') || column.id.includes('tanggal') ? 'date' : 'text'}
                              placeholder={
                                column.id.includes('datum') || column.id.includes('tanggal')
                                  ? 'Pilih tanggal'
                                  : 'Cari'
                              }
                              value={filterValues[column.id] || ''}
                ...
                return (
                  <tr {...row.getRowProps()} key={`tbody.tr${i}`}>
                    {row.cells.map((cell: any, indexTd: any) => {
                      // Extracting the values for `approvel_area` and `approvel_apd` from the row
                      const status_pekerjaan = row.original.status_pekerjaan;
                      const status_spk = row.original.status_nama;

                      

                      // Conditional style logic for background color
                      let backgroundColor = '';
                      
                      if (status_pekerjaan === 'RENCANA JADWAL PEKERJAAN') {
                        backgroundColor = 'white'; // Warna putih
                      } else if (status_pekerjaan === 'PELAKSANAAN') {
                        backgroundColor = '#FFFE99'; // Warna kuning
                      } else if (status_pekerjaan === 'SUDAH DILAKSANAKAN') {
                        backgroundColor = '#81C784'; // Warna hijau
                      } else if (status_pekerjaan === 'SUDAH MANUVER') {
                        backgroundColor = '#BBDEFB'; // Warna biru
                      } else if (status_pekerjaan === 'BATAL DILAKSANAKAN') {
                        backgroundColor = '#FA8072'; // Warna merah
                      }
                      if (status_spk === 'Draft') {
                        backgroundColor = 'white'; // Warna putih
                      } else if (status_spk === 'Disetujui') {
                        backgroundColor = '#81C784'; // Warna hijau
                      } else if (status_spk === 'Batal') {
                        backgroundColor = '#facc15'; // Warna merah
                      } else if (status_spk === 'Gagal') {
                        backgroundColor = '#ff0000ff'; // Warna kuning
                      }
                      ...
                      return (
                        <td
                          ...
                          style={{
                            ...
                            backgroundColor, // Terapkan warna latar belakang sesuai kondisi
                          }}
                        >
                          {cell.render('Cell')}
                        </td>
