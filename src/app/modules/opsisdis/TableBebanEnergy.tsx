...
import DataGrid, { Column, Editing, Lookup, Scrolling, Sorting } from 'devextreme-react/data-grid';
...
import { catchServiceMessage } from '@app/helper/app.helper';
/** COMPONENTS */
...
function TableBebanEnergy({
  ...
  const getAllData = async () => {
    ...
    try {
      ...
      let data: any = results?.map((item: any) => {
        item.nama_frek = item?.ref_frek?.nama
        item.date = moment(item?.datum).format("DD MMM YYYY HH:mm")
        return item;
      })
  ...
  const getGenerate = async () => {
    ...
    try {
      ...
      let message: string = catchServiceMessage(err, "Gagal Generate data");
    ...
    <>
      <div className='mb-3'>
        ...
        >
          ...
          <Column caption="Nama Alat" dataField="nama_frek" allowUpdating={false}
            allowEditing={false} />

          <Column dataField="id_ref_lokasi" caption="Lokasi" dataType='number' >
            <Lookup
              dataSource={optionsData}
              valueExpr='id_ref_lokasi'
              displayExpr='nama_lokasi'
            />
          </Column>
          <Column caption="Tanggal" dataField="date" allowUpdating={false}
            allowEditing={false} />

          <Column caption="Range Nilai" dataField="range_nilai" dataType='number' allowUpdating={false}
            allowEditing={false} />
...
export default TableBebanEnergy