...
export default function TableFrequensiEksekusi({
  pathService,
  primaryKey,
  caption,
  callBackCount,
    ...
    <>
      <div className='mb-3'>
        ...
        >
          ...
          <Column dataField='id_lokasi' caption={caption} dataType='number'>
            <Lookup
          ...
          {columnsConfig?.map((item: any, index: number) => (
            <Column dataField={item?.accessor} caption={item?.Header} key={index}></Column>
            // {
            //   item.columns?.map((items: any, ind: number) => (

            //     <Column dataField={items?.accessor} caption={items?.Header} key={ind}></Column>
            //   ))
