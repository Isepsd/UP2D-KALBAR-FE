...
import ReactTable from '@app/components/ReactTableNew';
...
import TopBarLoader from '@app/components/Loader/TopBarLoaderInLine';
...
export default function TableData({
  ...
  const handleFilterChange = (column: any, e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value || undefined;
  
    // Update local state filter
    setFilters((prev: any) => ({
      ...prev,
      [column.id]: value,
    }));
    ...
    // Set filter ke column (biasanya untuk tampilan lokal di tabel)
    ...
    // Set timeout baru dengan delay 1.5 detik
    ...
    if (selected) {
      switch (actionData) {
        ...
        case 'edit':
          ...
          break;
        case 'editentri':
          navigate(`editentri/${get(selected, primaryKey)}`);
          break;
        case 'detail':
  ...
  return (
    <>
<TopBarLoader isLoading={loading} position="inline" />
