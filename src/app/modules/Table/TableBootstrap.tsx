...
export default function TableBootstrap({
  ...
  /** GET DATA PAGINATION */
  const getGarduDistribusi = async (data_jar_har: any, params: any, paramsJarHar: any) => {
    if (request != true) { return false; }
    await new Promise((resolve) => setTimeout(resolve, 300));
    setLoading(true);
    ...
    try {
      ...
      const req: any = await getAllByPath(API_PATH().master.jaringan.ref_lokasi, prm, source.token);
      const { results } = req;
      setLoading(false);
      let gardu = []
      if (results.length > 0) {
    ...
    switch (type) {
      case 'opsis-jadwal-pemeliharaan':
        ...
        params = {
          ...
          pelaksana: dataSelected?.id_pelaksana ? dataSelected?.id_pelaksana : "",
  ...
  const insertTransJarHar = async (params: any) => {
    ...
    try {
      ...
      if (!dataSelected?.id_gardu) {
        getGarduDistribusi(resp?.results, {
  ...
  const insertTransJarHarDet = async (data_jar_har: any, gardu: any, paramsJarHar: any) => {
    let message;
      ...
      switch (actionData) {
        ...
        case 'update-status-listrik':
          ...
          setModalConfirm((prevState: any) => ({
            ...prevState,
            icon: 'fa-regular fa-circle-question',
            description: `Apakah status akan diubah ?`,
            subDescriotion: `Perubahan data status `,
            textApproved: 'Ok',
            classApproved: 'primary',
            textDecline: 'Cancel',
            show: true,
  ...
  const callbackModalConfirm = (approved = null) => {
    if (approved) {
      ...
      // console.log("dtble ", dataSelected);
  ...
  return (
    <>
      ...
      <Table className={containerClass} bordered>
        <thead>
          <tr>
            <th rowSpan={3}> A </th>
            <th> B </th>
