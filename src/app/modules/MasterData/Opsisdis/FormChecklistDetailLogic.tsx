...
import { FORM_CHECKLIST_DETAIL_LOGIC_COLUMN } from '@app/configs/react-table/master-opsisdis.columns.config';
import FormChecklistDetailLogicModal from './FormChecklistDetailLogicModal';

type Props = {
  itemSelected?: any,
};

const FormChecklistDetailLogic: FC<Props> = ({ itemSelected }) => {
  const dispatch = useDispatch();
  const source = axios.CancelToken.source();
  const path = API_PATH().master.opsisdis.pm.ref_pm_detail_logic
  ...
  const label = "LOGIC KE "
  const [columns] = useState<any>(FORM_CHECKLIST_DETAIL_LOGIC_COLUMN());
  ...
  const getAllData = async () => {
    ...
    try {
      const params = {
        ...
        id_ref_pm_detail: itemSelected?.id_ref_pm_detail ? itemSelected?.id_ref_pm_detail : "0",
      };
  ...
  useEffect(() => {
    if (itemSelected?.id_ref_pm_detail || pagination?.currentPage) {
      getAllData();
    }

  }, [pagination?.currentPage, itemSelected?.id_ref_pm_detail]);
    ...
    <>
      <Card>
        <Card.Header className='font-weight-600 align-items-center d-flex'>
          <div className='w-100'>Logic Ke {itemSelected?.nama}</div>
        ...
        <Card.Body>
          ...
          <ModalData modalProps={modal} ids="id_ref_pm_detail_logic">
            <FormChecklistDetailLogicModal
              id_ref_pm_detail={itemSelected?.id_ref_pm_detail}
...
export default FormChecklistDetailLogic