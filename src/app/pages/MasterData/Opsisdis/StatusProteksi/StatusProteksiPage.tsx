...
import { MASTER_STATUS_PROTEKSI } from '@app/configs/react-table/master-opsisdis.columns.config';
import BadgeStatus from '@app/components/Status/BadgeStatus';
import TableData from '@app/modules/Table/TableData';
import { ROLE_ACCESS, ROLE_ACTION } from '@app/helper/auth.helper';

export default function StatusProteksiPage() {
  const navigate = useNavigate();
  const path = API_PATH().master.opsisdis.rekap_padam.status_proteksi
  const [dataRows, setDataRows] = useState<any>([]);
  const [dataSelected, setDataSelected] = useState<any>();
  const [action, setAction] = useState<any>();

  const [columns, setColumns] = useState<any>(MASTER_STATUS_PROTEKSI());
  ...
  const handleEdit = (item: any) => {
    navigate(`edit/${item?.id_status_proteksi}`);
  };
