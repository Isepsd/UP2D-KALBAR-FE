...
import { FasopRTUField, IFasopRTU } from '@app/interface/fasop-rtu.interface';

function FasRTUFormPage() {
  const source = axios.CancelToken.source();
  const { activePage } = useSelector((state: any) => state.ui);
  const { id } = useParams();

  const dispatch = useDispatch();
  const path = API_PATH().master.fasop.rtu;
  ...
  const validationSchema = Yup.object().shape({
    id_ref_lokasi: Yup.number().nullable().transform((_, val) => val === Number(val) ? val : null),
    nama: Yup.string().required('Nama Wajib diisi'),
    b3text: Yup.string().nullable(),
    faktor: Yup.number().typeError('Faktor Wajib diisi').required('Faktor Wajib diisi'),
    status: Yup.number().nullable().transform((_, v) => (v == 1 ? 1 : 0)),
    id_pointtype: Yup.number().typeError('Jenis point Wajib diisi').required('Jenis point Wajib diisi'),
    kinerja: Yup.number().nullable().transform((_, v) => (v == 1 ? 1 : 0)),
    send_telegram: Yup.number().nullable().transform((_, v) => (v == 1 ? 1 : 0)),
  });
  ...
  const initDataForm = (data: any = undefined) => {
    const valueData =id && data  ? pick(data, getObjectKeys(FasopRTUField)) : FasopRTUField;
  ...
  const onSubmitForm = (data: IFasopRTU) => {
    upsertData(data);
  };
  ...
  const errorValidationHandling = (formInvalid:any)=>{
    if (typeof formInvalid=='object' && formInvalid instanceof Array == false) {
