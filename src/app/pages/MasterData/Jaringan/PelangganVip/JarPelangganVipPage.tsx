...
/** CONFIG */
import { PELANGGAN_VIP } from '@app/configs/react-table/master-jaringan.columns.config';
...
export default function JarPelangganVipPage() {
  ...
  const [columns, setColumns] = useState<any>(PELANGGAN_VIP());
  ...
  const handleRespDataApi = (data: any) => {
    ...
    data?.forEach((item: any, index: number) => {
      dataTableValue.push({
        ...
        daya_tersambung: item?.daya_tersambung,
        satuan_daya_tersambung: item?.satuan_daya_tersambung,
        ulp: item?.ulp,
        up3: item?.up3,
        ultg: item?.ultg,
  ...
  useEffect(() => {
    ...
    let roleAccess = ROLE_ACCESS("pelanggan-prioritas")
    const roleAct = {
  ...
  return (
    <>
      ...
      <TableData
        ...
        deleteConfirmation
        filterParams={{ sort_by: '-datum_created' }}
      ></TableData>
    </>
  );
}
