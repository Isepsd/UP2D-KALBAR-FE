...
import { Module } from '@app/configs/extmodule.config';
...
export default function RoleSettingPage(): React.ReactElement {
  ...
  const saveChangePrivileges = async () => {
    ...
    try {
      ...
      await putByPath(`${API_PATH().master.external.usertoken}`, params, id, source.token);
      setLoading(false);
      dispatchNotification('Berhasil mengubah pengaturan hak akses', 'success');

      /** CHECKING IF SAME WITH CURRENT USER */
      if (id && currentUser?.id_module == id) {
  ...
  const getAllDataMenu = async () => {
    ...
    try {
      ...
      const req: any = await getAllByPath(API_PATH().master.external.extmodule, params, source.token);

      const { results } = req;
      const dataResults = results ? results : [];
      const MODULE_RESP = dataResults?.map((d: any) => {
        return {
          ...d,
      ...
      const menus = initNestedMenu('', concat(MODULE_RESP, Module), null);
  ...
  const getDataById = async () => {
    try {
      const req: any = await getByIdPath(get(API_PATH(), "master.external.usertoken"), id, source.token);
  ...
  return (
    <>
                  ...
                  {item?.privileges?.length > 1 && (
                    <div className='form-check form-check-inline'>
                      <input
                        ...
                        checked={checkAllPriv(item?.namatoken, item?.privileges)}
                        onChange={(e) =>
                          checkAll(e, item?.namatoken, item?.privileges)
                        }
                      />
      ...
      <ActionFloating className='d-flex gap-2'>
        ...
        <Button variant='outline-secondary' type='reset' onClick={resetForm}>
          Batal
        </Button>
      </ActionFloating>
    </>
