...
function SigninPage({ isLoggedIn }: any) {
  ...
  const postSignin = async ({ username, password, rememberMe }: any) => {
    try {
      ...
      /** CEK url_dashboard */
      let path = reqUserDetail?.data?.url_dashboard || "/dashboard/dash-fasop/kinerja-scada";

      navigate(path); // Redirect ke URL dashboard atau default
    } catch (error: any) {
      setLoading(false);
  ...
  useEffect(() => {
    if (isLoggedIn) {
      const redirectPath = currentUser?.url_dashboard || '/dashboard/dash-fasop/kinerja-scada';
