...
export default function WmApprovalDetailPage() {
  const tabOptions = [
    {
      label: 'JSA',
      key: 'jsa',
  ...
  return (
    <>
      <Card className='card-widget'>
        ...
        <Card.Body>
          <WmApprovalDetailForm></WmApprovalDetailForm>
        </Card.Body>
      </Card>
      <Tabs
        defaultActiveKey='1'
      ...
      >
        {tabOptions.map((tab: any) => (
          <Tab key={tab.key} eventKey={tab.key} title={tab.label}>{tab?.component}</Tab>
        ))}
      </Tabs>
    </>
  );
