...
export default function WmApprovalClosePage() {
  const { closeModal } = useSelector((state: any) => state.ui);

  const navigate = useNavigate()
  /** DATA RESP */
  const [dataRows, setDataRows] = useState<any>([]);
  const [columns, setColumns] = useState<any>(WP_APPROVAL_COLUMNS()?.filter((f: any) => f?.accessor != 'reject'));
  ...
  useEffect(() => {
    let roleAccess = ROLE_ACCESS("approval-close")
    const roleAct = {
  ...
  return (
    <>
      ...
      <TableData
        ...
        trigger={triggers}
        approvalAlias={8}
        filterParams={{ sort_by: '-id_wp_online', status_persetujuan: '7' }}
        deleteConfirmation
      />
