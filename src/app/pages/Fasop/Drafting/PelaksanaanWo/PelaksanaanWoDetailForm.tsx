...
import { Col, Form, Modal ,Alert} from 'react-bootstrap';
...
function PelaksanaanWoDetailForm({st_selesai_wo,st_mulai_wo,dataSelected, handleClose,filterParams }: PelaksanaanWoDetailForm) {
  ...
  const onSubmitForm = (data: ICdn) => {
    if (st_mulai_wo?.st_mulai_wo === null) {
...
data
  ...
  return (
    <>
        {st_selesai_wo?.st_selesai_wo === 1 ? (
          <Alert variant="danger" className="text-center mb-4">
            <p>Data Yang Sudah Di Posting Tidak Bisa Upload Dokumen</p>
          </Alert>
        ) : st_mulai_wo?.st_mulai_wo === null ? (
          <Alert variant="danger" className="text-center mb-4">
            <p>Data Harus Di Ambil Terlebih Dahulu.</p>
          </Alert>
        ) : (
          st_mulai_wo?.st_mulai_wo === 1 && (
            <FormData
              setError={setError}
              setValue={setValue}
...
export default PelaksanaanWoDetailForm