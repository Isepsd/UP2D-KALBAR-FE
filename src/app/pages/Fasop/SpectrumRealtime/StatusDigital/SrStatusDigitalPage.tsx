...
export default function SrStatusDigitalPage() {
  /** DATA RESP */
  ...
  return (
    <>
      <TableDataListAction add={false} columns={columns} setColumns={setColumns}>
        <RealtimeFilter selectProps={{ fieldName: 'id_pointtype', pathServiceName: 'master.fasop.point_type', labelField: 'name', valueField: 'id_pointtype', placeholder: 'Pilih Jenis Point' }} queryParams={{page: -1, jenispoint: 'DIGITAL'}} isJenisPoint fieldKeyword='path3text' />
      </TableDataListAction> 

      <TableData columnsConfig={dataColumns} respDataApi={handleRespDataApi} rowData={dataRows} path={API_PATH().fasop.realtime.digital} primaryKey={'id'} deleteConfirmation />
    </>
  );
}
