...
import { initFlatMenu, initNestedMenu, initHiddenNestedMenu } from '@app/helper/menu.helper';
...
export default function MenuPage() {
  ...
  console.log ("hidden menu",hiddenMenus)
  ...
  // const watchHidden = useWatch({ control, name: 'hidden' });
  ...
  const getAllDataMenu = async () => {
    ...
    try {
      ...
      const hiddenMenusList = initHiddenNestedMenu('', MENU);
      ...
      setNestedMenu(menus);
      setHiddenMenus(hiddenMenusList);
  ...
  /** HANDLE UNHIDE MENU */
  const handleUnhide = async (menu: any) => {
  ...
  try {
    const params = {
      ...
      // ⬇️ WAJIB string
      privileges: Array.isArray(menu?.privileges)
        ? JSON.stringify(menu.privileges)
        : menu?.privileges,
    };

    await putByPath('menu', params, menu.id, source.token);

    setLoadingHidden(false);
    getAllDataMenu();

    dispatchNotification(
      `Sukses menampilkan menu "${menu.display}"`,
      'success'
    );
  } catch (err: any) {
    setLoadingHidden(false);
    dispatchNotification(
      `Gagal menampilkan menu "${menu.display}"`,
      'danger'
    );
  }
};
              ...
              <Card>
                ...
                <PerfectScrollbar style={{ height: 'calc(100vh - 310px)' }}>
                  <Card.Body>
                    ...
                    <Row className='mb-3'>
                      <Form.Group as={Col}>
                        <Form.Label>Hidden Menu</Form.Label>
                        <div>
                          <Form.Check
                            {...register('hidden')}
                            inline
            ...
            >
              ...
              >
                ...
                <small className='text-muted'>
                  Menu ini tidak muncul di navigasi karena di-hidden
                </small>
              </Card.Header>
