...
export default function AnomaliTindakLanjut() {
  const myGrid = useRef<JqxGrid>(null);
  const [showModal, setShowModal] = useState(false);
  const [isEdit, setIsEdit] = useState(false);
  const [viewMode, setViewMode] = useState(false);
  
  // Helper Tanggal[cite: 4]
    ...
    { 
      ...
      cellsrenderer: (row?: number) => `
        <div style="padding:10px 5px; text-align:center;">
          <button class="btn btn-xs btn-outline-primary me-1 btn-edit" data-row="${row}"><i class="fas fa-edit"></i> Tindak Lanjut</button>
        </div>`
    },
  ...
  return (
    <div onClick={handleAction}>
      <style>{`
        ...
        /* Grid Borders */
        .jqx-grid-column-header, .jqx-grid-cell, .jqx-widget-content, .jqx-grid-header { border-right: none !important; border-left: none !important; }
        .jqx-grid-cell { border-bottom: 1px solid #f2f2f2 !important; }
        .jqx-grid-header { border-bottom: 1px solid #eeeeee !important; }
        .jqx-grid-column-header { font-weight: bold !important; color: #444 !important; }
        .jqx-widget, .jqx-grid { border: none !important; }
        .jqx-fill-state-hover { background-color: #f9f9f9 !important; }
        .jqx-grid-filter-row { border: none !important; background-color: #ffffff !important; }
        .jqx-input { border: 1px solid #e0e0e0 !important; border-radius: 4px !important; }
      `}</style>

      {/* Header[cite: 4] */}
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h4 className="fw-bold mb-0" style={{ color: '#1e293b' }}>Daftar Tiket Anomali Peralatan - Perlu Tindak Lanjut</h4>
          <small className="text-muted">Klik <strong>Tindak Lanjut Tiket </strong> untuk merekam tindak lanjut anomali di lapangan.</small>
        </div>
      </div>
      ...
      {/* Filter Section[cite: 4] */}
      <Card className="filter-container shadow-sm mb-4">
        <Card.Body className="p-4">
          <Row className="align-items-end g-3">
            <Col md={2}><div className="filter-label">Dari Tanggal</div><Form.Control size="sm" type="date" value={startDate} onChange={e => setStartDate(e.target.value)} /></Col>
            <Col md={2}><div className="filter-label">Sampai Tanggal</div><Form.Control size="sm" type="date" value={endDate} onChange={e => setEndDate(e.target.value)} /></Col>
            <Col md={3}><div className="filter-label">Status Tiket</div>
      ...
      {/* MODAL FORM - PENAMBAHAN KOLOM SESUAI image_f3d170.png[cite: 4] */}
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" centered backdrop="static">
        <Modal.Header closeButton className="border-0 px-4 pt-4">
          <div>
            <h5 className="fw-bold mb-0">
              {isEdit ? 'Form Tindak Lanjut Pelaporan Anomali' : (viewMode ? 'Detail Pelaporan Anomali' : 'Form Pelaporan Anomali')}
            </h5>
          </div>
        </Modal.Header>
        <Modal.Body className="px-4 pb-4">
          <Form>
            {/* DATA TIKET AWAL (Logic Lama Dipertahankan)[cite: 4] */}
            <Row className="mb-3">
              <Col md={6}>
            ...
            <Row className="mb-3">
              <Col md={6}>
                ...
                <Form.Control disabled={viewMode} value={formData.pelapor} />
              </Col>
              <Col md={6}>
                <Form.Label>Pilih Aset</Form.Label>
                <Form.Control disabled={viewMode} value={formData.aset} readOnly />
              </Col>
            </Row>

            <Form.Group className="mb-3">
              <Form.Label>Deskripsi Temuan Anomali</Form.Label>
              <Form.Control as="textarea" rows={3} readOnly value={formData.deskripsi} />
            </Form.Group>

            {/* SECTION BARU: ENTRI TINDAK LANJUT SESUAI GAMBAR image_f3d170.png[cite: 4] */}
            <hr className="my-4" style={{ borderColor: '#e2e8f0' }} />
            
            <Form.Group className="mb-3">
              <Form.Label><i className="fas fa-lock me-1"></i> Tanggal Selesai (terkunci ke waktu sistem)</Form.Label>
              <Form.Control 
                type="text" 
                value="05 Mei 2026, 19.56" 
                readOnly 
                className="fw-bold text-muted"
              />
              <span className="helper-text">Strict no-backdate: waktu otomatis tersinkron ke sistem.</span>
            </Form.Group>

            <Form.Group className="mb-3">
              <Form.Label>Petugas Tindak Lanjut</Form.Label>
              <Form.Control 
                type="text" 
                placeholder="Contoh: Tim Pemeliharaan / Tim SCADA"
                defaultValue="Tim Pemeliharaan" 
              />
            </Form.Group>

            <Form.Group className="mb-3">
              <Form.Label>Hasil / Catatan Penyelesaian</Form.Label>
              <Form.Control 
                as="textarea" 
                rows={4} 
                placeholder="Contoh: Penggantian modul rele, pengujian ulang OK." 
              />
            </Form.Group>
            {/* END OF SECTION BARU[cite: 4] */}
