...
import DaftarPelaksanaanPemeliharaanFormPage from './DaftarPelaksanaanPemeliharaanFormPage'
import PelaksanaanFilter from './PelaksanaanFilter'
import DaftarPelaksanaaaPemeliharaanDetailPage from './DaftarPelaksanaanPemeliharaanDetailPage'
...
export default function DaftarPelaksanaanPemeliharaanPageJQ() {
  const originalDataRef = useRef<any[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [jadwal, setjadwal] = useState<any>([]);
  const [filterParams, setFilterParams] = useState<any>({ sort_by: '-updated_at', status_pekerjaan: 'PELAKSANAAN' });
  ...
  useEffect(() => {
    const roleAccess = ROLE_ACCESS("daftar-pelaksanaan-har");
    const roleAct = {
  ...
  return (
    <>
      ...
      <CardWidget title="FILTER">
        <PelaksanaanFilter onFilterChange={handleFilterChange} />
      </CardWidget>
          ...
          <Col md={12}>
            <JqxTabs theme="light">
              <ul style={{ marginLeft: 10 }} key="1">
                <li>
                  <i className="fa-solid fa-server"></i> Daftar Pelaksanaan Pemeliharaan
                </li>
              </ul>
              <div key="2">
                <TableDataJqxGridNew
      ...
      <Row>
        <Col md={12} className="mb-4">
          <DaftarPelaksanaaaPemeliharaanDetailPage filterParams={{ trans_jadwal_har_id: detailsHar, id_penyulang: penyulangID }} />
        </Col>
      </Row>

      <ModalFormWO modalProps={{ ...modalAdd, setShow: handleClose }}>
        <DaftarPelaksanaanPemeliharaanFormPage originalDataRef={originalDataRef} changesubmit={changesubmit} handleClose={handleClose} dataSelected={dataSelected} />
      </ModalFormWO>
