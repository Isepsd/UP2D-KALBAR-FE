...
import { OPSISDIS_DAFTAR_RENCANAHAR_COLUMN_JQX } from "@app/configs/react-table/opsisdis.column.config";
import TableDataJqxGridNew from '@app/modules/Table/TableDataJqxGridNew';
import CardWidget from '@app/components/Card/CardWidget';
import { Col, Row } from 'react-bootstrap';
import JqxTabs from 'jqwidgets-scripts/jqwidgets-react-tsx/jqxtabs';
import ModalFormWO from "@app/components/Modals/ModalFormWO";
import DaftarRencanaPemeliharaanFormPage from './DaftarRencanaPemeliharaanFormPage'
import RencanaFilter from './RencanaFilter'
import DaftarRencanaPemeliharaanDetailPage from './DaftarRencanaPemeliharaanDetailPage'
...
export default function DaftarRencanaPemeliharaanPageJQ() {
  const originalDataRef = useRef<any[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [jadwal, setjadwal] = useState<any>([]);
  const [filterParams, setFilterParams] = useState<any>({ sort_by: '-updated_at', status_pekerjaan: 'RENCANA JADWAL PEKERJAAN' });
  ...
  useEffect(() => {
    const roleAccess = ROLE_ACCESS("daftar-rencana-har");
    const roleAct = {
  ...
  return (
    <>
      ...
      <CardWidget title="FILTER">
        <RencanaFilter onFilterChange={handleFilterChange} />
      </CardWidget>
          ...
          <Col md={12}>
            <JqxTabs theme="light">
              <ul style={{ marginLeft: 10 }} key="1">
                <li>
                  <i className="fa-solid fa-server"></i> Daftar Rencana Pemeliharaan
                </li>
              </ul>
              <div key="2">
                <TableDataJqxGridNew
                  ...
                  dataFieldsColsConfig={OPSISDIS_DAFTAR_RENCANAHAR_COLUMN_JQX()}
      ...
      <Row>
        <Col md={12} className="mb-4">
          <DaftarRencanaPemeliharaanDetailPage filterParams={{ trans_jadwal_har_id: detailsHar, id_penyulang: penyulangID }} />
        </Col>
      </Row>

      <ModalFormWO modalProps={{ ...modalAdd, setShow: handleClose }}>
        <DaftarRencanaPemeliharaanFormPage originalDataRef={originalDataRef} changesubmit={changesubmit} handleClose={handleClose} dataSelected={dataSelected} />
      </ModalFormWO>
