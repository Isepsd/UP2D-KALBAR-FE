...
import DaftarRealisasiPemeliharaanFormPage from './DaftarRealisasiPemeliharaanFormPage'
import RealisasiFilter from './RealisasiFilter'
import DaftarRealisasiPemeliharaanDetailPage from './DaftarRealisasiPemeliharaanDetailPage'
...
export default function DaftarRealisasiPemeliharaanPageJQ() {
  const originalDataRef = useRef<any[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [jadwal, setjadwal] = useState<any>([]);
  const [filterParams, setFilterParams] = useState<any>({ sort_by: '-updated_at', status_pekerjaan: 'SUDAH DILAKSANAKAN' });
  ...
  useEffect(() => {
    const roleAccess = ROLE_ACCESS("daftar-realisasi-har");
    const roleAct = {
  ...
  return (
    <>
      ...
      <CardWidget title="FILTER">
        <RealisasiFilter onFilterChange={handleFilterChange} />
      </CardWidget>
          ...
          <Col md={12}>
            <JqxTabs theme="light">
              <ul style={{ marginLeft: 10 }} key="1">
                <li>
                  <i className="fa-solid fa-server"></i> Daftar Realisasi Pemeliharaan
                </li>
              </ul>
              <div key="2">
                <TableDataJqxGridNew
      ...
      <Row>
        <Col md={12} className="mb-4">
          <DaftarRealisasiPemeliharaanDetailPage filterParams={{ trans_jadwal_har_id: detailsHar, id_penyulang: penyulangID }} />
        </Col>
      </Row>

      <ModalFormWO modalProps={{ ...modalAdd, setShow: handleClose }}>
        <DaftarRealisasiPemeliharaanFormPage originalDataRef={originalDataRef} changesubmit={changesubmit} handleClose={handleClose} dataSelected={dataSelected} />
      </ModalFormWO>
