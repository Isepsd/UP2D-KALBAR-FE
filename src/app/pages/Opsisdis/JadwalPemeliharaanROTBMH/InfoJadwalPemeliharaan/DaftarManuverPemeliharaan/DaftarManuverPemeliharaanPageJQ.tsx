...
import DaftarManuverPemeliharaanFormPage from './DaftarManuverPemeliharaanFormPage'
import RencanaFilter from './RencanaFilter'
import DaftarManuverPemeliharaanDetailPage from './DaftarManuverPemeliharaanDetailPage'
...
export default function DaftarManuverPemeliharaanPageJQ() {
  const originalDataRef = useRef<any[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [jadwal, setjadwal] = useState<any>([]);
  const [filterParams, setFilterParams] = useState<any>({ sort_by: '-updated_at', status_pekerjaan: 'SUDAH MANUVER' });
    ...
    <>
      ...
      <Row>
        <Col md={12} className="mb-4">
          <DaftarManuverPemeliharaanDetailPage filterParams={{ trans_jadwal_har_id: detailsHar, id_penyulang: penyulangID }} />
        </Col>
      </Row>

      <ModalFormWO modalProps={{ ...modalAdd, setShow: handleClose }}>
        <DaftarManuverPemeliharaanFormPage originalDataRef={originalDataRef} changesubmit={changesubmit} handleClose={handleClose} dataSelected={dataSelected} />
      </ModalFormWO>
