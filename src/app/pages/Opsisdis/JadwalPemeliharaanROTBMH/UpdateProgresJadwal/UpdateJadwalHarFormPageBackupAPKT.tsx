...
function UpdateJadwalHarFormPage({
  ...
  useEffect(() => {
    console.log("Updated ID Apkt Trans Jar:", idApktTransJar);
  }, [idApktTransJar]);

  const onSubmitForm = async (data: IJadwalPemerliharaan) => {
    console.log("Cek seluruh data yang diterima:", data);
    ...
    // Inisialisasi paramss sebagai default params
    let paramss: any = { ...params };
    ...
    // Jika kondisi terpenuhi, ubah paramss
    if (data.status_pekerjaan === "PELAKSANAAN" && data.butuh_padam == 1) {
      console.log("Kondisi terpenuhi, mengubah paramss...");
      paramss = {
        ...params,
        lokasi: namaLokasi,
      ...
      console.log("Kondisi tidak terpenuhi, menggunakan params default.");
      paramss = params; // Gunakan params tanpa modifikasi tambahan
    }
    ...
    if (idApktTransJar) {
      setTimeout(async () => {
          ...
          data: [
            {
              ...
              id_apkt_trans_jar: idApktTransJar,  // ID yang telah disimpan
              tgl_padam: jambuka,  // ID yang telah disimpan
        ...
        const response2: any = await postByPath(
          get(API_PATH(), "apkt.trans_jar_detail"),
          paramsDetail,
          source.token
        );
