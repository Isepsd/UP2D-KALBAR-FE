...
/** CONFIG */
import { OPSISDIS_POSTING_JADWALHAR_ROTBMH_COLUMN } from "@app/configs/react-table/opsisdis.column.config";
import { timeFormatSec } from "@app/helper/time.helper";
import PostingJadwalHarFormPage from "./KepencetJadwalHarFormPage";
/** COMPONENTS */
import TableData from "@app/modules/Table/TableData";
import TableDataListAction from "@app/modules/Table/TableDataListActionDetailPage";
import PostingFilter from "./KepencetFilter";
import { ROLE_ACCESS, ROLE_ACTION } from "@app/helper/auth.helper";
/** SERVICE */
import PostingJadwalHarDetailPageCopy from "./KepencetJadwalHarDetailPageCopy";
import { API_PATH } from "@app/services/_path.service";
import PostingForm from "./KepencetForm";
...
export default function SrStatusRTUPage() {
  ...
  const [columns, setColumns] = useState<any>(
    OPSISDIS_POSTING_JADWALHAR_ROTBMH_COLUMN()
  );
  ...
  const [detailRelease, setdetailRelease] = useState<any>();
  // const [detailPostingPost, setdetailPostingPost] = useState<any>();
  const [detailApprove, setdetailApprove] = useState<any>();
  // console.log('ada', detailPosting)
  const [action, setAction] = useState<string>();
  ...
  const getAllData = async () => {
    ...
    try {
      ...
      const req: any = await getAllByPath(
        API_PATH().admin.role_entri_jadwal,
        params,
        source.token
      );
      ...
      if (req?.results) {
        ...
        } else if (results.id_roles === id_roles) {
          // Jika results adalah objek langsung
          setid_area(results.id_ref_lokasi_up3);
        } else {
          setid_area(null);
        }
      } else {
  ...
  //   return {
  //     min: targetMonday.startOf("day"),
  //     max: targetMonday.clone().add(6, "days").endOf("day"),
  //   };
  // };

  // const canPosting = (item: any) => {
  //   const kategori = item?.kategori_rotbmh;
  //   const tanggalUsulan = item?.tanggal; 
  //   const periodeAwal = item?.periode_awal;
  //   const periodeAkhir = item?.periode_akhir;
  //   const releaseP = item?.release_rotbmh;

  //   // ❌ tanpa tanggal → tidak boleh
  //   if (!tanggalUsulan) return false;

  //   // ✅ selain ROT & ROB → bebas posting
  //   if (kategori !== "ROT" && kategori !== "ROB") {
  //     return true;
  //   }

  //   if (kategori === "ROT" && kategori === "ROB" && (releaseP === 0 || releaseP === '' || releaseP === null)) {
  //     return false;
  //   }

  //   // ⛔ ROT / ROB → cek window ROM
  //   const { min, max } = getRangeDateROM();

  //   const tglUsulan = moment(tanggalUsulan);
  //   const tglAwal = periodeAwal ? moment(periodeAwal) : null;
  //   const tglAkhir = periodeAkhir ? moment(periodeAkhir) : null;

  //   const isTanggalValid =
  //     tglUsulan.isBetween(min, max, "day", "[]") &&
  //     (!tglAwal || tglAwal.isBetween(min, max, "day", "[]")) &&
  //     (!tglAkhir || tglAkhir.isBetween(min, max, "day", "[]"));
  ...
  const handleRespDataApi = (data: any) => {
    ...
    data?.forEach((item: any) => {
      ...
      dataTableValue.push({
          ...
          <div className="d-flex gap-2">
            ...
            // && canPosting(item)
             && (
              ...
              >
                <i className="fas fa-paper-plane"></i> Cancel Batal
              </button>
            )}
          </div>
        ),
        ...
        datum_updated:
          item?.approvel_area === 1
            ? `${timeFormatSec(item?.datum_updated)}`
            : "-", // Tampilkan hanya ketika approvel_area = 1
      });
    });
  ...
  //   setdetailPostingPost(item?.approvel_apd);
  ...
  const handlePosting = (item: any) => {
    ...
    if ((item.current?.kategori_rotbmh === "ROT" || item.current?.kategori_rotbmh === "ROB") && (item.current?.release_rotbmh === 0 || item.current?.release_rotbmh === '')) {
      alert("Data ROT ROB harus direlease dulu.");
      return; // Stop further execution if already posted
    }
    ...
    setdetailRelease(item.release_rotbmh);
  ...
  useEffect(() => {
    ...
    let roleAccess = ROLE_ACCESS("kepencet-jadwal-har");
    const roleAct = {
    ...
    <>
      ...
      <TableDataListAction
        ...
        filterParams={{
          ...
          datum_before: moment()
            ...
            .format("YYYY-MM-DD"),
          status_pekerjaan_in:
            "DRAFT,USULAN PEKERJAAN,RENCANA JADWAL PEKERJAAN,USULAN PEKERJAAN (APPROVED UID), PELAKSANAAN, SUDAH DILAKSANAKAN, SUDAH MANUVER, BATAL DILAKSANAKAN, RELEASE PEKERJAAN",
        }}
      ></TableDataListAction>
      <CardWidget>
        <PostingFilter />
      ...
      <Row>
        <Col md={12} className="mb-4">
          <PostingJadwalHarDetailPageCopy
            filterParams={{
      ...
      <ModalFormWO modalProps={{ ...modalPosting, setShow: handleClose }}>
        <PostingForm
          ...
          isKategori={detailKategori}
          isRelease={detailRelease}
      ...
      >
        <PostingJadwalHarFormPage
          handleClose={handleClose}
          dataSelected={dataSelected}
          isAlreadyApproved={detailPostingPost}
        />
      </ModalFormWO> */}
    </>
  );
